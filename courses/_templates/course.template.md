...
```mermaid
flowchart LR
    BEFORE[Capacité précédente] --> CURRENT[Module Mx]
    CURRENT --> AFTER[Capacité suivante]
```
...
```mermaid
flowchart TB
    USER[Apprenant / opérateur] --> TOOL[Outil]
    TOOL --> TARGET[(Ressource cible)]
    TOOL --> STATE[(État / preuve)]
```
