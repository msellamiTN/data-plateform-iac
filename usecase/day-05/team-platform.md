# 🧪 Lab J5 — Team PLATFORM · Fares · Mohamed · Sirine
## 🎯 Mission
...
|---|---|---|
| **Fares** | **future grants** sur les schemas de Data Engineering | le droit qui s'applique aux objets **pas encore créés** |
| **Mohamed** | **hiérarchie de rôles** — `GB_ANALYST` hérite de `GB_MART_READER` | l'héritage de privilèges |
| **Sirine** | **resource monitor** avec seuils | 💰 la limite de dépense **exécutable** |

> 🔵 **C'est vous qui construisez le pipeline de la salle** *(étape ③, 10 h 45 – 11 h 45)*. Les trois autres équipes le déclenchent et lisent son résultat — c'est ainsi que cela se passe en entreprise.
> 🔴 **Au cleanup, Platform détruit en DERNIER.**
...
|---|---|
| `source = "../../modules/rbac"` | `source = "git::https://…/globalbank-modules-platform//rbac?ref=v1.0.0"` |
...
|---|---|---|
| **Fares** | des **future grants** | le droit qui s'applique aux objets **pas encore créés** |
| **Mohamed** | une **hiérarchie de rôles** | l'héritage de privilèges |
| **Sirine** | un **resource monitor** avec seuils | 💰 la limite de dépense **exécutable** |

---

### 🔵 Fares — les *future grants*

#### ① 🖱️ D'abord, à la main

**Snowsight → Worksheets.** Vous avez accordé `SELECT` sur les trois tables d'Amal mercredi. **Amal en crée une quatrième ce matin.**

```sql
-- la nouvelle table d'Amal
SHOW GRANTS ON TABLE GB_RAW_DB.LANDING.TRANSACTIONS_V2;
```

**Aucun grant.** `GB_RAW_READER` ne la voit pas.

> 🛑 **Un grant est une photographie, pas une règle.** Il s'applique aux objets qui existaient au moment où vous l'avez écrit. Chaque nouvelle table exige un nouveau grant — c'est ingérable.

```sql
GRANT SELECT ON FUTURE TABLES IN SCHEMA GB_RAW_DB.LANDING TO ROLE GB_RAW_READER;
```

#### ② 🔍 Vérifiez

```sql
SHOW FUTURE GRANTS IN SCHEMA GB_RAW_DB.LANDING;
```

| grant_on | privilege | grantee_name |
|---|---|---|
| TABLE | SELECT | GB_RAW_READER |
...
| SQL | Terraform |
|---|---|
| `GRANT SELECT` | `privileges = ["SELECT"]` |
| `ON FUTURE TABLES` | bloc `future` avec `object_type_plural = "TABLES"` |
| `IN SCHEMA GB_RAW_DB.LANDING` | `in_schema = "\"GB_RAW_DB\".\"LANDING\""` |
| `TO ROLE GB_RAW_READER` | `account_role_name = …` |

#### ④ ⌨️ En Terraform

Révoquez d'abord le grant manuel, puis ajoutez à votre module `rbac/` :

```sql
REVOKE SELECT ON FUTURE TABLES IN SCHEMA GB_RAW_DB.LANDING FROM ROLE GB_RAW_READER;
```

```hcl
resource "snowflake_grant_privileges_to_account_role" "future_tables" {
  account_role_name = snowflake_account_role.access_roles["raw_reader"].name
  privileges        = ["SELECT"]

  on_schema_object {
    future {
      object_type_plural = "TABLES"
      in_schema          = "\"${var.target_database}\".\"LANDING\""
    }
  }
}
```

> 🧠 **Les guillemets échappés ne sont pas décoratifs.** Snowflake attend un identifiant pleinement qualifié et **cité** : `"GB_RAW_DB"."LANDING"`. Sans les guillemets, un nom contenant un caractère spécial casserait la ressource.

> 💡 **Si votre version du provider refuse la syntaxe**, consultez le Registry : les ressources de grants ont beaucoup évolué entre les versions majeures.

#### 🧪 La preuve

**Demandez à Amal de créer une table de plus**, puis :

```sql
SHOW GRANTS ON TABLE GB_RAW_DB.LANDING.<nouvelle_table>;
```

`GB_RAW_READER` y figure — **sans que vous ayez rien fait.**

---

### 🔵 Mohamed — la hiérarchie de rôles

#### ① 🖱️ D'abord, à la main

**Snowsight → Admin → Users & Roles → Roles.** Le graphe montre des rôles **isolés** : chacun doit recevoir tous ses privilèges un par un.

```sql
GRANT ROLE GB_MART_READER TO ROLE GB_ANALYST;
```

Rafraîchissez le graphe : une flèche est apparue.

#### ② 🔍 Vérifiez

```sql
SHOW GRANTS TO ROLE GB_ANALYST;
```

`GB_ANALYST` **hérite** de tout ce que `GB_MART_READER` possède, présent et futur.

> 🧠 **Le modèle Snowflake à deux étages :**
>
> | Étage | Rôle | Ce qu'il porte |
> |---|---|---|
> | **Accès** | `GB_MART_READER` | des privilèges sur des objets |
> | **Fonctionnel** | `GB_ANALYST` | des **rôles d'accès**, jamais de privilèges directs |
>
> On donne aux personnes des rôles **fonctionnels**. On ne leur donne jamais de privilège directement.

#### ③ 🔁 La correspondance

| SQL | Terraform |
|---|---|
| `GRANT ROLE <A> TO ROLE <B>` | `snowflake_grant_account_role` |
| `<A>` — le rôle accordé | `role_name` |
| `<B>` — celui qui hérite | `parent_role_name` |

#### ④ ⌨️ En Terraform

```sql
REVOKE ROLE GB_MART_READER FROM ROLE GB_ANALYST;
```

```hcl
variable "role_hierarchy" {
  type        = map(string)
  description = "rôle fonctionnel => rôle d'accès dont il hérite"

  default = {
    analyst  = "GB_MART_READER"
    business = "GB_CORE_READER"
    engineer = "GB_RAW_READER"
  }
}
...
resource "snowflake_grant_account_role" "hierarchy" {
  ...
  parent_role_name = snowflake_account_role.functional_roles[each.key].name
}
```

> 🎯 **Trois lignes de données décrivent tout le modèle de sécurité de GlobalBank.** Ajouter un rôle métier demain, c'est une ligne dans la map.

#### 🧪 Le test négatif — celui qui compte

```sql
USE ROLE GB_ANALYST;
SELECT * FROM GB_RAW_DB.LANDING.ACCOUNTS LIMIT 1;
```

```
SQL access control error:
Insufficient privileges to operate on schema 'LANDING'
```

> 🏆 **C'est le résultat attendu.** `GB_ANALYST` voit les marts, **pas la zone brute**. **Un test de sécurité qui réussit est un test qui échoue à accéder.**

---

### 🔵 Sirine — le resource monitor

#### ① 🖱️ D'abord, à la main

**Snowsight → Admin → Cost Management → Resource Monitors → `+ Resource Monitor`**

```
   ┌─ Create Resource Monitor ───────────────────────────┐
   │   Name              GB_MON_PLATFORM           ──── ①
   │   Credit quota      5                         ──── ②
   │   Monitor type      Account / Warehouse       ──── ③
   │   Schedule          Monthly, starts immediately ── ④
   │                                                     │
   │   Actions                                           │
   │     Notify           at  50 %                 ──── ⑤
   │     Notify           at  75 %                       │
   │     Suspend          at  90 %                 ──── ⑥
   │     Suspend immediately at 100 %              ──── ⑦
   └─────────────────────────────────────────────────────┘
```

| Action | Ce qu'elle fait vraiment |
|---|---|
| ⑤ **Notify** | Envoie un mail. **N'arrête rien.** |
| ⑥ **Suspend** | Laisse **finir** les requêtes en cours, puis suspend |
| ⑦ **Suspend immediately** | **Tue** les requêtes en cours |

> 💰 **Le trio 75 / 90 / 100 est le réglage de production.** On prévient, puis on freine en douceur, puis on coupe. Un monitor qui ne fait que notifier n'a jamais empêché une facture.

#### ② 🔍 Vérifiez

```sql
SHOW RESOURCE MONITORS LIKE 'GB_MON_%';
```

#### ③ 🔁 La correspondance

| Champ Snowsight | Terraform |
|---|---|
| Name | `name` |
| Credit quota | `credit_quota` |
| Schedule = Monthly | `frequency = "MONTHLY"` + `start_timestamp = "IMMEDIATELY"` |
| Notify at 50 %, 75 % | `notify_triggers = [50, 75]` |
| Suspend at 90 % | `suspend_trigger = 90` |
| Suspend immediately at 100 % | `suspend_immediate_trigger = 100` |

#### ④ ⌨️ En Terraform

Supprimez le monitor manuel, puis dans votre module `compute/` :

```hcl
resource "snowflake_resource_monitor" "platform" {
  name            = "${var.prefix}_MON_PLATFORM${local.suffix}"
...
resource "snowflake_warehouse" "service" {
  ...
  resource_monitor  = snowflake_resource_monitor.platform.name   # ← le rattachement
  auto_resume       = true
...
> 💰 **La ligne `resource_monitor` est celle qui compte.** Un monitor sans warehouse rattaché ne surveille rien du tout — c'est l'erreur la plus fréquente en production.

> 💡 **Les noms d'attributs des triggers ont changé selon les versions du provider.** Si `plan` refuse, vérifiez la page `snowflake_resource_monitor` du Registry pour votre version.

#### ⑤ 🧠 Le fondement de l'étape — pour toute l'équipe

| Fondement | En une phrase |
|---|---|
| **Future grants** | Un droit qui s'applique aux objets **pas encore créés** — la seule façon de tenir une plateforme qui grandit. |
| **Hiérarchie de rôles** | Les personnes reçoivent des rôles **fonctionnels** ; les privilèges vivent sur les rôles **d'accès**. |
| **Le test négatif** | Un contrôle de sécurité se prouve par ce qu'il **refuse**. |
| 💰 **Resource monitor** | La limite de dépense devient **exécutable** — et rattachée, sinon elle ne sert à rien. |
