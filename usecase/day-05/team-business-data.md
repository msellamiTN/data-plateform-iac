# 🧪 Lab J5 — Team BUSINESS DATA · Manel · Leila · Olfa
## 🎯 Mission
...
|---|---|---|
| **Manel** | **masking policy** sur une colonne sensible | la donnée qui se protège elle-même |
| **Leila** | **tag de gouvernance** appliqué à sa database | la classification qui vit **sur** l'objet |
| **Olfa** | **future grants** + vue de publication | le droit qui précède l'objet |

> 🟠 **La boucle se ferme aujourd'hui.** `var.classification` est validée depuis mardi, elle est devenue une entrée de module jeudi, et elle sera ce soir **inscrite sur l'objet lui-même**, requêtable par l'auditeur.
...
|---|---|
| `source = "../../modules/data-domain"` | `source = "git::https://…/globalbank-modules-business-data//data-domain?ref=v1.0.0"` |
...
|---|---|---|
| **Manel** | une **masking policy** | la donnée qui se protège elle-même |
| **Leila** | un **tag de gouvernance** | la classification qui vit **sur** l'objet |
| **Olfa** | des **future grants** + une vue de publication | le droit qui précède l'objet |

---

### 🟠 Manel — la masking policy

#### ① 🖱️ D'abord, à la main

**Snowsight → Worksheets.** Votre table `CUSTOMER` contient une colonne `LABEL` qui porte le nom du client. **Tout rôle ayant `SELECT` la lit en clair.**

```sql
USE ROLE GB_ANALYST;
SELECT ID, LABEL FROM GB_CUSTOMER_DB.BUSINESS.CUSTOMER LIMIT 3;
```
...
> 🛑 **Un analyste n'a aucun besoin du nom en clair pour compter des clients par segment.** C'est le principe de minimisation du RGPD, et il se code.

```sql
CREATE MASKING POLICY GB_MASK_NAME AS (val STRING) RETURNS STRING ->
  CASE
    WHEN CURRENT_ROLE() IN ('GB_BUSINESS', 'ACCOUNTADMIN') THEN val
    ELSE '***MASQUÉ***'
  END;

ALTER TABLE GB_CUSTOMER_DB.BUSINESS.CUSTOMER
  MODIFY COLUMN LABEL SET MASKING POLICY GB_MASK_NAME;
```

#### ② 🔍 Le test qui prouve — deux rôles, deux résultats

```sql
USE ROLE GB_BUSINESS;
SELECT ID, LABEL FROM GB_CUSTOMER_DB.BUSINESS.CUSTOMER LIMIT 3;   -- en clair

USE ROLE GB_ANALYST;
SELECT ID, LABEL FROM GB_CUSTOMER_DB.BUSINESS.CUSTOMER LIMIT 3;   -- ***MASQUÉ***
```

> 🏆 **La même requête, la même table, deux résultats.** Le masquage est appliqué **par le moteur**, pas par l'application. Aucun rapport, aucun export, aucun accès direct ne peut le contourner.

#### ③ 🔁 La correspondance

| SQL | Terraform |
|---|---|
| `CREATE MASKING POLICY` | `snowflake_masking_policy` |
| `AS (val STRING)` | bloc `argument` avec `name` et `type` |
| `RETURNS STRING` | `return_data_type = "STRING"` |
| le corps `CASE …` | `body` |
| `ALTER TABLE … SET MASKING POLICY` | bloc `masking_policy` sur la colonne |

#### ④ ⌨️ En Terraform

Retirez d'abord la policy manuelle :

```sql
ALTER TABLE GB_CUSTOMER_DB.BUSINESS.CUSTOMER MODIFY COLUMN LABEL UNSET MASKING POLICY;
DROP MASKING POLICY GB_MASK_NAME;
```

Puis, dans votre module `data-domain/` :

```hcl
resource "snowflake_masking_policy" "name_mask" {
  name     = "${var.prefix}_MASK_NAME${local.suffix}"
  ...
  body             = <<-SQL
    CASE
      WHEN CURRENT_ROLE() IN ('${var.unmasked_role}') THEN VAL
      ELSE '***MASQUÉ***'
    END
  SQL

  comment = "Minimisation RGPD | ${local.owner_tag}"
}
```

> 🧠 **`<<-SQL … SQL` est un *heredoc*.** Il permet d'écrire un bloc de texte multi-lignes en HCL ; le tiret après `<<` autorise l'indentation. C'est la façon propre d'embarquer du SQL dans du Terraform.

> 🧠 **`var.unmasked_role` plutôt qu'un nom en dur.** Le rôle autorisé est fourni par Platform : c'est **encore un contrat entre équipes**.

> 💡 **Le rattachement de la policy à la colonne** dépend de votre version du provider : soit un bloc `masking_policy` dans la `column`, soit une ressource `snowflake_table_column_masking_policy_application`. Consultez le Registry.

---

### 🟠 Leila — le tag de gouvernance

#### ① 🖱️ D'abord, à la main

**Snowsight → Data → Governance → Tags** *(ou en SQL)*

```sql
CREATE TAG GB_CLASSIFICATION
  ALLOWED_VALUES 'PUBLIC', 'INTERNE', 'CONFIDENTIEL', 'SECRET'
  COMMENT = 'Classification GlobalBank';

ALTER DATABASE GB_PRODUCT_DB SET TAG GB_CLASSIFICATION = 'INTERNE';
```

#### ② 🔍 Vérifiez — et mesurez la portée

```sql
SELECT * FROM TABLE(
  SNOWFLAKE.ACCOUNT_USAGE.TAG_REFERENCES('GB_PRODUCT_DB', 'DATABASE')
);
```

> 🧠 **Depuis mardi, votre classification vivait dans un `comment`.** Un commentaire ne se requête pas, ne se filtre pas, ne s'audite pas. **Un tag, si** — et il **descend automatiquement** sur tous les objets de la database.

> ⏱️ **`ACCOUNT_USAGE` a une latence** — jusqu'à deux heures. Si votre requête ne renvoie rien tout de suite, ce n'est pas une erreur.

#### ③ 🔁 La correspondance

| SQL | Terraform |
|---|---|
| `CREATE TAG` | `snowflake_tag` |
| `ALLOWED_VALUES` | `allowed_values = [ … ]` |
| `ALTER DATABASE … SET TAG` | `snowflake_tag_association` |

#### ④ ⌨️ En Terraform

```hcl
resource "snowflake_tag" "classification" {
  name           = "${var.prefix}_CLASSIFICATION${local.suffix}"
  database       = snowflake_database.this.name
  schema         = snowflake_schema.this.name
  allowed_values = ["PUBLIC", "INTERNE", "CONFIDENTIEL", "SECRET"]
  comment        = "Classification GlobalBank | ${local.owner_tag}"
}

resource "snowflake_tag_association" "db" {
  object_type = "DATABASE"
  tag_id      = snowflake_tag.classification.fully_qualified_name
  tag_value   = var.classification        # ← la variable validée du Jour 2

  object_identifier {
    name = snowflake_database.this.name
  }
}
```

> 🎯 **La boucle est bouclée.** `var.classification` est validée depuis mardi, elle est devenue une entrée de module mercredi, et elle est aujourd'hui **inscrite sur l'objet lui-même**, requêtable par l'auditeur. **C'est un data product gouverné.**

> 💡 **La forme de `snowflake_tag_association` varie selon les versions** *(`object_identifier` en bloc ou en liste)*. Consultez le Registry.

---

### 🟠 Olfa — future grants et vue de publication

#### ① 🖱️ D'abord, à la main

```sql
CREATE VIEW GB_CAMPAIGN_DB.BUSINESS.V_CAMPAIGN_PERFORMANCE AS
SELECT c.ID, c.LABEL, COUNT(r.ID) AS RESPONSES
FROM GB_CAMPAIGN_DB.BUSINESS.CAMPAIGN c
LEFT JOIN GB_CAMPAIGN_DB.BUSINESS.CAMPAIGN_RESPONSE r ON r.ID = c.ID
GROUP BY 1, 2;

GRANT SELECT ON FUTURE VIEWS IN SCHEMA GB_CAMPAIGN_DB.BUSINESS TO ROLE GB_ANALYST;
```

#### ② 🔍 Vérifiez

```sql
SHOW FUTURE GRANTS IN SCHEMA GB_CAMPAIGN_DB.BUSINESS;
```

> 🧠 **`SHOW GRANTS` ne les montre pas — il faut `SHOW FUTURE GRANTS`.** Deux catalogues distincts : les droits d'aujourd'hui, et la règle pour demain.

#### ③ 🔁 La correspondance

| SQL | Terraform |
|---|---|
| `CREATE VIEW` | `snowflake_view` + `statement` |
| `ON FUTURE VIEWS IN SCHEMA` | bloc `future` avec `object_type_plural = "VIEWS"` |
| `TO ROLE GB_ANALYST` | `account_role_name = var.consumer_role` |

#### ④ ⌨️ En Terraform

```hcl
resource "snowflake_view" "performance" {
  name      = "V_CAMPAIGN_PERFORMANCE"
  database  = snowflake_database.this.name
  schema    = snowflake_schema.this.name
  comment   = "Performance des campagnes | ${local.owner_tag}"

  statement = <<-SQL
    SELECT c.ID, c.LABEL, COUNT(r.ID) AS RESPONSES
    FROM ${snowflake_database.this.name}.BUSINESS.CAMPAIGN c
    LEFT JOIN ${snowflake_database.this.name}.BUSINESS.CAMPAIGN_RESPONSE r
      ON r.ID = c.ID
    GROUP BY 1, 2
  SQL
}
...
resource "snowflake_grant_privileges_to_account_role" "future_views" {
  account_role_name = var.consumer_role
  privileges        = ["SELECT"]

  on_schema_object {
    future {
      object_type_plural = "VIEWS"
      in_schema          = "\"${snowflake_database.this.name}\".\"BUSINESS\""
    }
  }
}
```

> 🧠 **Le nom de la database est interpolé dans le SQL de la vue.** En UAT, la vue pointera automatiquement sur `GB_CAMPAIGN_DB_UAT`. **Un nom en dur ici aurait fait lire les données de production depuis la recette** — c'est l'incident classique.

#### ⑤ 🧠 Le fondement de l'étape — pour toute l'équipe

| Fondement | En une phrase |
|---|---|
| **Masking policy** | La protection vit **sur la donnée**, pas dans l'application. |
| **Tag** | La classification devient **requêtable et auditable**, contrairement à un commentaire. |
| **Future grants** | Le droit précède l'objet — indispensable sur un domaine qui grandit. |
| **Interpoler, jamais coder en dur** | Une vue avec un nom en dur lit la production depuis la recette. |
| **Le rôle autorisé est une variable** | La sécurité aussi passe par un contrat entre équipes. |
