# 🧪 Lab J1 — Team DATA ENGINEERING · Amal · Lara · Fares
...
> **Je veux** créer ma database, mon schema et ma table, d'abord à la main puis en Terraform
...
|---|---|---|---|
| **Amal** | `APP04_RAW_DEV` | `LANDING` | `ACCOUNTS` |
| **Lara** | `APP05_CORE_DEV` | `DIM` | `DIM_CUSTOMER` |
| **Fares** | `APP01_RAW_DEV` | `LANDING` | `ACCOUNTS` |

> 🧠 `APP04` est **votre préfixe apprenant** — déjà configuré sur votre poste. Il rend vos ressources uniques.
...
3. Name : `APP04_RAW_DEV` *(votre nom du tableau)* → **Create**.
...
2. Name : `LANDING` → **Create**.

### 1c. La table

1. Cliquez sur votre schema → **Create → Table**.
2. Name : `ACCOUNTS`, avec 2 colonnes :
...
variable "learner_prefix" {
  type        = string
  description = "Votre préfixe unique (ex: APP04) — déjà assigné"
...
1. Dans **Snowsight**, renommez le commentaire de votre table : `Modifié à la main`.
2. `terraform plan` :

```text
  ~ comment = "Modifié à la main" -> "Managed by Terraform | Training | APP04"
```
...
1. Dans `main.tf`, changez le nom du schema `LANDING` → `STAGING`.
2. `terraform plan` → observez : le schema sera **remplacé** (`-/+`), et la table aussi (elle dépend du schema).
3. **Ne validez pas.** Remettez `LANDING` → `No changes.`
...
> Ajoutez une **deuxième table** `TRANSACTIONS` dans le même schema, avec les colonnes `ID` (NUMBER) et `AMOUNT` (NUMBER(38,2)).
