# 🧪 Lab J3 — Team BUSINESS DATA · Manel · Leila · Olfa
...
|---|---|---|
| **Manel** | `tables` → 3 tables dans `APP06_CUSTOMER_DEV.BUSINESS` | `APP06_LEGACY` (database) |
| **Leila** | `tables` → 3 tables dans `APP07_PRODUCT_DEV.BUSINESS` | `APP07_LEGACY` (database) |
| **Olfa** | `tables` → 3 tables dans `APP08_CAMPAIGN_DEV.BUSINESS` | `APP08_LEGACY` (database) |
...
**Problème :** je veux renommer mon bloc `snowflake_table.collection` en `snowflake_table.business`. Si je change juste le nom du bloc, Terraform voit : *« détruire `collection`, créer `business` »* → les tables sont détruites puis recréées. Sur des tables avec des données, c'est un incident.
...
# APRÈS : le bloc s'appelle "business"
resource "snowflake_table" "business" {
  for_each = var.tables
...
# Je dis à Terraform : "collection" et "business" sont LE MÊME objet
moved {
  from = snowflake_table.collection
  to   = snowflake_table.business
}
```
...
```text
  # snowflake_table.collection["customer"] has moved to
  # snowflake_table.business["customer"]
...
**Problème :** au Jour 1, j'ai créé `APP06_LEGACY` **à la main** dans Snowsight. Il existe dans Snowflake mais pas dans mon code. C'est une **ressource orpheline**.
...
```text
  # module.mes_tables.snowflake_table.this["customer"] will be created
  # snowflake_table.business["customer"] will be destroyed
```
...
**Problème :** Business Data doit connaître la database RAW de Data Eng pour préparer les flux. Mais je ne l'ai pas créée — je ne peux pas la mettre dans un `resource`.
...
> Ajoutez une table `CUSTOMER_SCORE` via `terraform.tfvars` — **sans toucher au module ni à `main.tf`**.
