# 🧪 Lab J3 — Team DATA ENGINEERING · Amal · Lara · Fares
...
|---|---|---|
| **Amal** | `tables` → 3 tables dans `APP04_RAW_DEV.LANDING` | `APP04_LEGACY` (database) |
| **Lara** | `tables` → 3 tables dans `APP05_CORE_DEV.DIM` | `APP05_LEGACY` (database) |
| **Fares** | `tables` → 3 tables dans `APP01_RAW_DEV.LANDING` | `APP01_LEGACY` (database) |
...
**Problème :** je veux renommer mon bloc `snowflake_table.collection` en `snowflake_table.landing`. Si je change juste le nom du bloc, Terraform voit : *« détruire `collection`, créer `landing` »* → les tables sont détruites puis recréées. Sur des tables avec des données, c'est un incident.
...
# APRÈS : le bloc s'appelle "landing"
...
# Je dis à Terraform : "collection" et "landing" sont LE MÊME objet
moved {
  from = snowflake_table.collection
  to   = snowflake_table.landing
}
```
...
```text
  # snowflake_table.collection["accounts"] has moved to
...
**Problème :** au Jour 1, j'ai créé `APP04_LEGACY` **à la main** dans Snowsight. Il existe dans Snowflake mais pas dans mon code. C'est une **ressource orpheline**.
...
```powershell
terraform import snowflake_database.legacy APP04_LEGACY
```
...
module "mes_tables" {
  ...
  tables         = var.tables                        # entrée : ma collection
  database       = snowflake_database.ma_db.name     # entrée : ma database
...
```text
  # module.mes_tables.snowflake_table.this["accounts"] will be created
  # snowflake_table.landing["accounts"] will be destroyed
```
...
**Problème :** Data Eng doit connaître le warehouse de Platform pour exécuter ses requêtes. Mais je ne l'ai pas créé — je ne peux pas le mettre dans un `resource`.
...
```hcl
# Je LIS le warehouse de Mohamed — je ne le crée pas, je ne le gère pas
data "snowflake_warehouses" "ingest" {
  like = "WH_APP02_BUSINESS_DEV"   # le warehouse de l'équipe Platform
}
```
...
terraform apply
terraform output warehouse_lu
```

```text
"WH_APP02_BUSINESS_DEV"
```
...
> Ajoutez une table `WIRE_TRANSFERS` via `terraform.tfvars` — **sans toucher au module ni à `main.tf`**.
