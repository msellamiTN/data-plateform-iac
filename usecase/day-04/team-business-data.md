# 🧪 Lab J4 — Team BUSINESS DATA · Manel · Leila · Olfa
## 🎯 Mission
...
|---|:---:|---|---|---|
| **Manel** | `APP06` | `modules/data-domain/` | database + schema + `customer_tables` | `GB_CUSTOMER_DB_UAT` |
| **Leila** | `APP07` | `modules/data-domain/` | database + schema + `product_tables` | `GB_PRODUCT_DB_UAT` |
| **Olfa** | `APP08` | `modules/data-domain/` | database + schema + `campaign_tables` | `GB_CAMPAIGN_DB_UAT` |

> 🟠 **Vous écrivez trois fois le même module.** C'est voulu : à 15 h 30, vous comparerez vos trois versions et vous n'en garderez qu'une. **C'est exactement ce qui se passe dans une vraie équipe plateforme.**
...
```
   ...
   └── envs/
       ├── dev/             ← key = APP06/dev.tfstate
       └── uat/             ← key = APP06/uat.tfstate
```
