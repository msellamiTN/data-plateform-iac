# 🧪 Lab J4 — Team DATA ENGINEERING · Amal · Lara
## 🎯 Mission
...
|---|:---:|---|---|---|
| **Amal** | `APP04` | `modules/landing-zone/` | database + schema + `raw_tables` | `GB_RAW_DB_UAT` |
| **Lara** | `APP05` | `modules/landing-zone/` | database + schema + `dim_tables` | `GB_CORE_DB_UAT` |

> 🟢 **Vous écrivez le même module, chacune de votre côté.** À 15 h 30, vous comparerez vos deux versions et vous n'en garderez qu'une — c'est la séance de convergence, et c'est le moment fort de la journée.
> 💰 **Aucun warehouse en UAT.** Une database vide ne coûte rien.
...
```
   ...
   └── envs/
       ├── dev/             ← key = APP04/dev.tfstate
       └── uat/             ← key = APP04/uat.tfstate
```
