...
for i in $(seq 1 $LEARNER_COUNT); do
    ...
    if $USER_EXISTS; then
        if $RESET_PASSWORDS; then
            ...
            if snow sql -c training -q "ALTER USER $NAME SET PASSWORD = '<REDACTED>'" >/dev/null 2>&1; then
        ...
        if snow sql -c training -q "CREATE USER $NAME PASSWORD = '<REDACTED>' DEFAULT_ROLE='SYSADMIN' MUST_CHANGE_PASSWORD=FALSE" >/dev/null 2>&1; then
