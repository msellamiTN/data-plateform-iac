...
foreach ($l in $learners) {
    ...
    if ($userExists) {
        if ($ResetPasswords) {
            ...
            $prevEAP = $ErrorActionPreference
            $ErrorActionPreference = 'Continue'
            $resetResult = & snow sql -c training -q "ALTER USER $($l.Name) SET PASSWORD = '<REDACTED>'" 2>&1
        ...
        $prevEAP = $ErrorActionPreference
        $ErrorActionPreference = 'Continue'
        $createResult = & snow sql -c training -q "CREATE USER $($l.Name) PASSWORD = '<REDACTED>' DEFAULT_ROLE='SYSADMIN' MUST_CHANGE_PASSWORD=FALSE" 2>&1
