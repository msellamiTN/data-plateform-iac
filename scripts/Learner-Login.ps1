...
.DESCRIPTION
    Triple-mode authentication for learners:

    Mode 0 — Managed Identity (preferred on VMs with system-assigned MI):
      1. Login to Azure with az login --identity (non-interactive)
      2. Fetch SP credentials + SnowflakePAT from Key Vault
      3. Logout MI, then login with SP (for Terraform provider auth)
      4. Set all ARM_* and TF_VAR_snowflake_token environment variables

    Mode 1 — KV-first (preferred when MI not available):
    ...
    The script tries Mode 0 first (non-interactive). If MI is not available,
    it tries Mode 1 (interactive). If that fails or -ForceFallback is set,
    it falls back to Mode 2.
...
# ------------------------------------------------------------------
# Mode 0 — Managed Identity (preferred on VMs with system-assigned MI)
# Non-interactive: no browser, no SP secret needed for initial login.
# Works even with -ForceFallback (MI is non-interactive, like fallback).
# ------------------------------------------------------------------
$kvFirstSuccess = $false
$spCreds = @{}
$patValue = $null

# Check if local secrets files exist — if so, skip MI and browser login modes
...
if ($hasLocalSecrets) {
    Write-Host '[INFO] Local secrets files found — skipping Managed Identity and browser login modes.' -ForegroundColor Green
}

if ($kvName -and -not $hasLocalSecrets) {
    Write-Host '[INFO] Trying managed identity login...' -ForegroundColor DarkGray

    $prevEAP = $ErrorActionPreference
    $ErrorActionPreference = 'Continue'
    $miLoginResult = & az login --identity --output none 2>&1
    ...
    if ($miLoginExit -eq 0) {
        ...
        # Fetch SP credentials from Key Vault (for Terraform provider auth)
        ...
        if ($missing.Count -eq 0) {
            ...
            $spContent = @(
                "# Auto-generated by Learner-Login.ps1 (MI mode)"
        ...
        # Logout MI session so SP login below works cleanly
        & az logout 2>&1 | Out-Null
    } else {
        Write-Host '[INFO] Managed identity not available.' -ForegroundColor DarkGray
    }
    Write-Host ''
}
...
# ------------------------------------------------------------------
if (-not $kvFirstSuccess -and -not $ForceFallback -and $kvName -and -not $hasLocalSecrets) {
